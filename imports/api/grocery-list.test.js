...
if(Meteor.isServer){
  describe('groceryLists', function () {
    ...
    it('should insert new grocery list', function () {
      const userId = listOne.userId
      const _id = Meteor.server.method_handlers['groceryLists.insert'].apply({ userId: userId });

      expect(GroceryLists.find({ _id, userId })).toExist();
    });
    ...
    it('should remove a list', function () {
      Meteor.server.method_handlers['groceryLists.remove'].apply({ userId: listOne.userId}, [listOne._id]);
    ...
    it('should not remove list if not authenticated', function () {
      expect(() => {
        Meteor.server.method_handlers['groceryLists.remove'].apply({}, [listOne._id]);
      }).toThrow();
    })

    it('should not remove a list if invalid id', function () {
      expect(() => {
        Meteor.server.method_handlers['groceryLists.remove'].apply({ userId: 'testUserId1'});
      }).toThrow();
    })
    ...
    it('should not update if extra updates', function () {
      expect(() => {
        Meteor.server.method_handlers['groceryLists.update'].apply({
          ...
          listOne._id,
            { listName: "new name", nameTest: 'Camila' }
        ]);
      }).toThrow();
    });
    ...
    it('should return a users lists', function () {
      const res = Meteor.server.publish_handlers.groceryLists.apply({ userId: listOne.userId });
      const lists = res.fetch();

      expect(lists.length).toBe(1);
      expect(lists[0]).toEqual(listOne)
    });

    it('should retunr no notes for user that has none', function () {
      const res = Meteor.server.publish_handlers.groceryLists.apply({ userId: 'randomUser' });
      const lists = res.fetch();
